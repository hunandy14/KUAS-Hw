...
// 印所有節點的資料 (鏈結開頭,模式)
// 模式1 = 含開頭
void node_printalldata(node* nh, int mode);
// 印所有節點的資料 (鏈結開頭)
...
//=========================================================
// 檢查河內塔移動n1移動到n2 (柱子地址,柱子地址)
int hanoi_checkmove(node* nh1, node* nh2);
// 河內塔移動n1移動到n2 (柱子地址,柱子地址)
...
// 紀錄河內塔移動步驟 (柱子地址)
void hanoi_setuplog(node* nh1){
    // 輸入起點柱子，將數值儲存在開頭
    nh1->data = nodep_final(nh1)->data;
    // cout << "setuplog = " <<nh1->data << endl;
}
// 確認移動不重複 (柱子地址)
int hanoi_checkre(node* nh1, node* nh2){
    // cout << "ht[0] = " << nh1+1 <<endl;
    // 輸入目的柱子，檢查是否不重複
    if (nh2->data != nodep_final(nh1)->data){
        return 1;
    } else{
        cout << "  **remove" << endl;
        return 0;
...
void hanoi_move(node** ht, node* nh1, node* nh2){
    if (hanoi_checkmove(nh1, nh2)==0)
        return; //確認後不可移動
    if (hanoi_checkre(nh1, nh2) ==0){
        return; //確認後發現重複
    }
    ht[0]-> data = 0;
    ht[1]-> data = 0;
    ht[2]-> data = 0;
    hanoi_setuplog(nh1);
    return hanoi_move(nh1, nh2);
}

// 河內塔移動n1移動到n2 (河內塔地址,柱子,柱子)
void hanoi_move(node** ht, int n1, int n2){
    return hanoi_move(ht, ht[n1], ht[n2]);
}
//=========================================================
int main(int argc, char const *argv[]){
    /* 開頭節點 */
    node* *ht = new node*[3]; // 3個柱子
    /* 批次匯入節點 */
    int len = 4, *data = new int[len]; // 盤子數量
    for (int i = 0; i < len; ++i){
        ht[i] = node_creat(0);
        data[i] = len-i;
    ...
    // cout << "ht[1] = " << ht[1] <<endl;
    // cout << "ht[2] = " << ht[2] <<endl;
    // cout << "ht add = " << *(ht+0) <<endl;
    // cout << "ht add = " << *(ht+1) <<endl;
    // cout << "ht add = " << *(ht+2) <<endl;
    cout << "===============測試" << endl;

    // hanoi_move(ht,ht[0],ht[2]);
    // hanoi_move(ht,ht[0],ht[1]);
    // hanoi_move(ht,ht[0],ht[1]);
    // hanoi_move(ht,0,1);
    // hanoi_move(ht,0,1);
    // hanoi_move(ht[0],ht[2]);

    // hanoi_checkre(ht[0],ht[1]);
    // node_printall(ht);
    //=====================================================
...
void hanoi_receive(node** ht, int mode){
    if (mode==0){ //小收回
        if (nodep_final(ht[1])->data > \
            ...
            hanoi_move(ht, ht[2],ht[0]);
        }else{
            hanoi_move(ht, ht[1],ht[0]);
        }
        return;
    }
    if (mode==1){ //大收回
...
void hanoi_defrag(node** ht){
    ...
    if (hanoi_handdif(ht,-1) == 1)
        hanoi_move(ht, ht[2],ht[1]);
    else if (hanoi_handdif(ht,-1) == -1)
        hanoi_move(ht, ht[1],ht[2]);
    else
        cout << "defrag Error!" << endl;
}
